...
private:
    ...
    std::map<Node, SE2> positions2Dav;
    std::map<Node, SE3> positions3D;
    std::map<Node, SE3> positions3Dav;
    ...
    void runWeightedMASAT2D();
    void runWeightedMASAT3D();

    double heuristic2D(Edge2D edge, SE2 currPos, SE2 neighborPos);
    double heuristic3D(Edge3D edge, SE3 currPos, SE3 neighborPos);
};
