...
void Graph::runMASAT(){
    if(D==2) {runMASAT2D(); runWeightedMASAT2D();}
    else if(D==3) {runMASAT3D(); runWeightedMASAT3D();}
...
void Graph::runWeightedMASAT2D(){
    ...
    auto fst = structure2D.begin();
    Node source = fst->first;
    std::cout<<"Running MASAT weighted average from VERTEX "<<source.getID()<<"."<<std::endl;
    ...
    while (!Q.empty()) {
        ...
        // calculate a weighted average of the results from all edges
        double s = 0;
        double sx = 0;
        double sy = 0;
        double ssin = 0;
        ...
        for (auto e : structure2D[current]) {
            Node neighbor(e.getFromID());

            // add to queue
            if (!marked.at(neighbor)) {
                Q.push(neighbor);
            ...
            sx += weight * guess[0];
            sy += weight * guess[1];
            scos += weight * cos(guess[2]);
            ssin += weight * sin(guess[2]);
        ...
        if (s > 0) {
            ...
            positions2Dav[current] = positions2D[current];
        }
...
double Graph::heuristic2D(Edge2D edge, SE2 currPos, SE2 neighborPos) {
    ...
    double dx = neighborPos[0] - currPos[0];
    double dy = neighborPos[1] - currPos[1];
    double distance = sqrt(dx * dx + dy * dy);
    double distanceWeight = 1.0 / (distance + epsilon);

    // combine the inverse noise weight and distance
    return (distanceWeight * noiseWeight);
}

void Graph::runWeightedMASAT3D(){
    std::map<Node, bool> marked;
    for(auto p : structure3D) {
        marked[p.first] = false;
    }
...
double Graph::heuristic3D(Edge3D edge, SE3 currPos, SE3 neighborPos) {
    // prevent division by 0
    double epsilon = 0.0001;
    ...
    double dx = neighborPos[0] - currPos[0];
    double dy = neighborPos[1] - currPos[1];
    double dz = neighborPos[2] - currPos[2];
